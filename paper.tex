...
\end{enumerate}
While each option stands for itself, in reality, an institutional RSE department will most certainly finance its personnel by an appropriate mixture of possibly all four options. The mixture at a particular institution depends heavily on the local conditions.
...
\subsubsection{Creation of the department} \label{sec:creation_of_dep}
Once the concept has been approved by the institution, the RSE department can be created accordingly.
The initial staffing depends mostly on the local institutional conditions. 
One promising possibility is to start with two positions. The first one would be an RSE coordinator.
They will be the contact person for all institutional RSEs (whether belonging to the hub department or to individual research groups, i.e., decentralised network of RSEs in the institution), organising meetings, developing training programs, reporting to superordinate bodies, etc.
The second position would be a central RSE, responsible for providing selected services and infrastructure.
These central positions will be complemented by the existing RSEs organised in the network to form a pool of institutional RSEs associated with the department.
...
An extension of the initial service portfolio for a larger target audience requires the acquisition of further funding and installation of further positions, see below (\autoref{sec:growth_of_dep}).

\subsubsection{Internal communication of the department}
...
Although an initial announcement will be done once the the RSE department concept has been approved by the institution, communication across the RSE hub, the RSE network and other departments should be done on a regular basis. 
Such a communication should facilitate collaboration among RSEs while also keeping non-RSEs in the loop. 

For communication among RSEs, some options include messaging channels (for quick communication) and forums or wikis integrated to developer platforms to have longer discussion while keeping track of the conversation, e.g.,  via issues. 
Project management communication tools, e.g., Kanban boards, could complement the picture. 

With regards to the broader community, i.e., departments within the corresponding institute, a web/wiki page including the portfolio of services and training materials around best RSE practices, would be ideal. 
This in fact could be one of the initial infrastructure tasks carried out by the second RSE previously described (see \autoref{sec:creation_of_dep}). 
To communicate and advertise the web/wiki page within the institute, the corresponding team for internal communication (the people responsible for internal news, for instance) should be involved in this process.
In addition, and to emphasize the role in research of RSE, sharing the work done by the RSE department and the network RSEs as part of, for instance, institutional research colloquia should become a common practice.

\subsubsection{Growth of the department} \label{sec:growth_of_dep}
